...
static void print_file(char st[]);
...
/**
 * To open the logfile and output to it.
 */
static void print_file(char st[]){
if(output_file){
...
// out of the queue	
		...
		//waiting for a job from the job_queue
		sem_wait(&this->job_queue->num_jobs);
		//putting a lock
		pthread_mutex_lock(&this->job_queue->lock);
		//accessing the job
		job=this->job_queue->head;
		//moving the pointer head to the next job
		this->job_queue->head=job->next_job;
		//unlock
		pthread_mutex_unlock(&this->job_queue->lock);
		//to print the file
		if(job->file_name != NULL){
		printer_print(&(this->driver),job);
		}
		//to the get system time for logfile
		time_t systime;
    		systime = time(NULL);
				
		print_file("File printed");
		print_file("\n");
    		print_file(ctime(&systime));
    		print_file("\n");
...
//#warning The student should push the job into the queuel
...
// be consumed.	
				...
				//system time for logfile
				time_t systime;
    				systime = time(NULL);
    				print_file(ctime(&systime));
				print_file("\n");
				print_file(job->job_name);
				print_file("\n");
				print_file(job->group_name);
				print_file("\n");
				print_file(job->file_name);
