...
void SimTask::OnStartTick(unsigned long sys_time)
{
	...
	if(this->next_arrival_time == sys_time)
	{
		...
		case SimTask::NOT_ARRIVED:
			Debug::AppendLog
			(std::string("Task ") + this->name + " arrived at " + std::to_string((long long unsigned int)sys_time));
		...
		case SimTask::BLOCKED:
			Debug::AppendLog(std::string("Task ") + this->name + " unblocked at " + std::to_string((long long unsigned int)sys_time));
			this->last_state = this->state;
			this->state = SimTask::READY;
			Scheduler::AddTask(this, sys_time);
			//this->task->MoveReady();
...
void SimTask::OnSysTick(unsigned long sys_time)
{
	...
	if(this->state == SimTask::RUNNING)
	{
		...
		if(this->times[this->times_index] == 0)
		{
			...
			if(this->times[this->times_index] > 0)
			{
				Debug::AppendLog(std::string("Task ") + this->name + " blocked at " + std::to_string((long long unsigned int)sys_time));
				this->last_state = this->state;
			...
			{
				Debug::AppendLog(std::string("Task ") + this->name + " finished at " + std::to_string((long long unsigned int)sys_time));
...
std::string SimTask::GetLogString()
{
	std::string str("");
	str += "Task " + this->name + ": ";
	str += "priority=" + std::to_string((long long unsigned int)task_info.priority) + ", ";
	str += "start=" + std::to_string((long long unsigned int)task_info.time_arive) + ", ";
	str += "finish=" + std::to_string((long long unsigned int)finish_time) + ", ";
	str += "deadline=" + std::to_string((long long unsigned int)task_info.deadline) + ", ";
	str += "runtime=" + std::to_string((long long unsigned int)task_info.run_time) + ", ";
	str += "blocktime=" + std::to_string((long long unsigned int)task_info.block_time) + ", ";
	str += "switches=" + std::to_string((long long unsigned int)num_context_switches) + ", ";
	str += "bursts=" + std::to_string((long long unsigned int)num_bursts);
	return str;
}
